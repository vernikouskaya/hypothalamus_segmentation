...
images_path = './GT' # series of graound truth hypothalamus masks
masks_path = './pred' # series of predicted masks
img_overlay_path = './overlay'
img_overlay_GT_path = './overlay_GT'
...
while i<=len(images):
    ...
    mask_color_underlay = color.label2rgb(original_mask, colors=['red'], image=img_half, alpha=0.7, bg_label=0)
    new_image_GT_path = "%s/%s.png" % (img_overlay_GT_path, images_name[i - 1])
    io.imsave(new_image_GT_path, mask_color_underlay)


    mask_color = color.label2rgb(original_mask, bg_label=0)
